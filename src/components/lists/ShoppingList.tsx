...
export function ShoppingList({ tripId }: { tripId: string }) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const categories = new Set<string>(
          data.items.map((i: ShoppingItem) => i.category)
        )
  ...
  const toggleItem = async (item: ShoppingItem) => {
    ...
    try {
      await fetch(`/api/trips/${tripId}/shopping?itemId=${item.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const deleteItem = async (itemId: string) => {
    ...
    try {
      await fetch(`/api/trips/${tripId}/shopping?itemId=${itemId}`, {
        method: "DELETE",
      })
    } catch (error) {
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center gap-4 rounded-xl border border-zinc-200 bg-white p-10 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
        <p className="text-zinc-500">אין פריטים ברשימת הקניות</p>
        <button
    ...
    <div className="flex flex-col gap-4">
              ...
              <div className="border-t border-zinc-100 dark:border-zinc-700">
                ...
                <div className="flex items-center gap-2 border-t border-zinc-100 px-4 py-2 dark:border-zinc-700">
                  ...
                  <button
                    ...
                    className="rounded-md bg-green-50 px-3 py-1 text-xs font-medium text-green-600 hover:bg-green-100 disabled:opacity-40 transition-colors dark:bg-green-900/20 dark:text-green-400 dark:hover:bg-green-900/40"
                  >
                    הוסף
                  </button>
                </div>
      ...
      {showNewCategory ? (
        <div className="rounded-xl border border-dashed border-zinc-300 bg-white p-4 shadow-sm dark:border-zinc-600 dark:bg-zinc-800">
          <div className="flex flex-col gap-2">
            <input
              ...
              className="rounded-md border border-zinc-200 bg-transparent px-3 py-1.5 text-sm text-right outline-none focus:border-green-500 dark:border-zinc-600"
              dir="rtl"
              autoFocus
            />
            <input
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                ...
                className="rounded-md bg-green-600 px-3 py-1 text-xs font-medium text-white hover:bg-green-700 disabled:opacity-40 transition-colors"
              >
                הוסף קטגוריה
              </button>
            </div>
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-xl border border-dashed border-zinc-300 py-3 text-sm text-zinc-500 hover:border-green-400 hover:text-green-500 transition-colors dark:border-zinc-600"
        >
